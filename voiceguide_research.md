...
거리 측정 모델(Depth Anything V2)을 Android에서 직접 돌리는 방법도 기술적으로는 가능하지만, 실사용에서 의미가 없음.

- GPU 서버 기준 0.3초/장인데, 모바일 CPU에서는 1~2초 이상으로 느려짐
- YOLO까지 같이 돌리면 한 프레임에 2~3초 → 장애물 회피에 치명적인 딜레이
- 시각장애인 실내 이동 보조에서 2초 딜레이는 안전 문제로 직결됨

**결론: 거리 측정은 서버에서 처리, Android는 이미지 전송만 담당.**  
STT·TTS도 이미 인터넷이 필요한 구조라 기존 제약과 동일함.
