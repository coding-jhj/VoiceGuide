from __future__ import annotations

import argparse
import csv
import json
from collections import Counter, defaultdict
from pathlib import Path

import cv2


ROOT = Path(__file__).resolve().parents[1]
IMAGE_SUFFIXES = {".jpg", ".jpeg", ".png", ".webp", ".bmp"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate YOLO predictions on VoiceGuide door/stairs regression sets.")
    parser.add_argument("--model", default="models/voiceguide82_yolo11n.pt")
    parser.add_argument("--images", default="data/test_images")
    parser.add_argument("--out", default="outputs/voiceguide82_eval")
    parser.add_argument("--conf", type=float, default=0.05)
    parser.add_argument("--classes", default="person,chair,cell phone,stairs,door")
    return parser.parse_args()


def iter_images(root: Path) -> list[Path]:
    return sorted(p for p in root.rglob("*") if p.is_file() and p.suffix.lower() in IMAGE_SUFFIXES)


def main() -> int:
    args = parse_args()
    from ultralytics import YOLO

    image_root = Path(args.images)
    out_dir = Path(args.out)
    out_dir.mkdir(parents=True, exist_ok=True)
    images = iter_images(image_root)
    if not images:
        print(f"no images under {image_root}")
        return 1

    model = YOLO(args.model)
    watched = {name.strip() for name in args.classes.split(",") if name.strip()}
    rows = []
    class_counts: Counter[str] = Counter()
    folder_counts: dict[str, Counter[str]] = defaultdict(Counter)

    for image in images:
        mat = cv2.imread(str(image))
        if mat is None:
            continue
        result = model(mat, conf=args.conf, verbose=False)[0]
        rel = image.relative_to(image_root).as_posix()
        folder = image.parent.name
        if not result.boxes:
            rows.append({"image": rel, "folder": folder, "class": "", "conf": "", "xyxy": ""})
            continue
        for box in result.boxes:
            cls_id = int(box.cls[0])
            name = str(model.names.get(cls_id, cls_id))
            conf = float(box.conf[0])
            xyxy = [round(float(v), 2) for v in box.xyxy[0]]
            if watched and name not in watched:
                continue
            class_counts[name] += 1
            folder_counts[folder][name] += 1
            rows.append({
                "image": rel,
                "folder": folder,
                "class_id": cls_id,
                "class": name,
                "conf": f"{conf:.4f}",
                "xyxy": json.dumps(xyxy),
            })

    csv_path = out_dir / "predictions.csv"
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["image", "folder", "class_id", "class", "conf", "xyxy"])
        writer.writeheader()
        writer.writerows(rows)

    summary = {
        "model": args.model,
        "images": len(images),
        "classes": dict(class_counts),
        "by_folder": {folder: dict(counts) for folder, counts in sorted(folder_counts.items())},
        "notes": [
            "For hard-negative folders such as hanging clothes, person detections should be rare.",
            "For door/stairs folders, check recall at a class-specific threshold before Android export.",
        ],
    }
    summary_path = out_dir / "summary.json"
    summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"csv: {csv_path}")
    print(f"summary: {summary_path}")
    print(json.dumps(summary, ensure_ascii=False, indent=2))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
