...
경로 정보 (OSRM 보행자 경로 — 안전 우회 경로):
  거리  : 약 1,303m  (직선 최단 1,160m 대비 +143m)
  소요  : 약 17분 (실제 보행 속도 기준)
  특징  : 신대방동 사고다발 구간 우회 + 음향신호기·보행자작동신호기 횡단보도(06-0000032157) 경유
  시뮬레이션: STEPS_PER_SEG=6, INTERVAL=1.0초 → 약 5분 소요
...
STEPS_PER_SEG = 6        # OSRM 구간당 보간 점수  →  53구간×6×1초 ≈ 5분 20초
DETECT_EVERY  = 4        # N번째 GPS 전송마다 탐지 데이터 전송
LOOP          = False
ZONE_SIZE     = 6        # osrm_idx // ZONE_SIZE = zone_idx (9개 존)

# ── OSRM 실제 보행 경로 (54개 좌표, 1,303m) ──────────────────────────────────
# 경로: 동광교회(성대로1가길) → 신대방동 사고다발 우회 → 음향신호기 횡단보도 → 복지관
# routing.openstreetmap.de/routed-foot 로 사전 계산된 실제 인도 경로
OSRM_COORDS = [
    (37.501727, 126.935007), (37.501583, 126.93445),  (37.501415, 126.934372),
    (37.501026, 126.934016), (37.50086,  126.933684), (37.500626, 126.933025),
    (37.50037,  126.932667), (37.500078, 126.932357), (37.499765, 126.932067),
    (37.499743, 126.932037), (37.499534, 126.931771), (37.499264, 126.931221),
    (37.499244, 126.93118),  (37.499177, 126.930975), (37.499162, 126.930915),
    (37.499048, 126.930481), (37.498979, 126.930097), (37.498952, 126.929951),
    (37.498893, 126.929152), (37.498442, 126.929108), (37.498399, 126.929067),
    (37.498277, 126.929209), (37.498206, 126.929276), (37.498243, 126.929332),
    (37.498234, 126.929694), (37.498243, 126.929332), (37.498206, 126.929276),
    (37.498056, 126.929409), (37.497851, 126.929571), (37.497775, 126.929659),
    (37.497746, 126.929619), (37.497561, 126.929247), (37.497503, 126.92905),
    (37.497489, 126.929003), (37.497457, 126.928887), (37.497173, 126.928546),
    (37.497157, 126.928472), (37.497095, 126.928489), (37.496914, 126.928523),
    (37.49677,  126.928523), (37.496607, 126.928543), (37.496514, 126.92854),
    (37.496351, 126.928517), (37.496248, 126.928483), (37.49618,  126.92846),
    (37.496053, 126.928408), (37.495993, 126.928307), (37.4961,   126.927666),
    (37.495937, 126.92763),  (37.495292, 126.927471), (37.495038, 126.92741),
    (37.495034, 126.927409), (37.495038, 126.92741),  (37.495096, 126.927424),
]

# 9개 존(zone_idx 0~8) + 도착(zone 9) 라벨
WAYPOINT_LABELS = [
    "출발: 성대로1가길 동광교회",              # zone 0 (osrm 0-5)
    "상도로 보도 이동",                       # zone 1 (osrm 6-11)
    "골목길 진입",                           # zone 2 (osrm 12-17)
    "사고다발 위험구간 우회로 진입",           # zone 3 (osrm 18-23)
    "신대방동 안전 우회 구간",                # zone 4 (osrm 24-29)
    "여의대방로 방면",                        # zone 5 (osrm 30-35)
    "복지관 방면 이동",                       # zone 6 (osrm 36-41)
    "음향신호기 횡단보도 대기 (경로 B 선택)",  # zone 7 (osrm 42-47)
    "횡단 후 복지관 방면",                    # zone 8 (osrm 48-53)
...
WAYPOINT_SCENES: dict[int, list[dict] | None] = {
    ...
    2: [      # 골목길 — 자동차·사람
        _obj("자동차", 0.90, 0.50, 0.60, 0.38, 0.55, 5.2),
        _obj("사람",   0.78, 0.28, 0.52, 0.14, 0.60, 2.8),
    ],
    3: [      # 사고다발 우회로 진입 — 오토바이 주의 (고위험 구간)
    ...
    5: [      # 여의대방로 방면 — 신호등·자동차
        _obj("신호등", 0.94, 0.50, 0.28, 0.05, 0.16, 8.0),
        _obj("자동차", 0.89, 0.55, 0.65, 0.36, 0.56, 6.5),
        _obj("버스",   0.82, 0.72, 0.62, 0.30, 0.54, 9.0),
    ],
    6: [      # 복지관 방면 이동 — 사람·핸드백
...
def run():
    ...
    print(f"  OSRM 좌표: {len(OSRM_COORDS)}개, 약 1,303m")
